  ...
  async function updateStatus(status: string) {
    ...
    try {
      const response = await fetch(
        `/api/employees/documents/${documentId}`,
        {
          method: "PATCH",
          headers: {
      ...
      if (!response.ok) {
        throw new Error(
          result?.message || "Unable to update document status."
        );
      }

      setSuccess(
        result?.message || "Document verification status updated."
      );
    ...
    <div className="mt-6">
      <div className="rounded-xl border border-slate-800 bg-slate-950 p-5">
        ...
        <textarea
          ...
          placeholder="Enter remarks if required, especially when rejecting a document."
          rows={4}
        ...
        <p className="mt-2 text-xs text-slate-500">
          Rejection requires remarks. For verification, remarks are
          optional.
        </p>
      </div>
