...
export default function NewEmployeeSalaryPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result?.message || "Unable to add salary record."
        );
      }

      setSuccess(
        result?.message || "Salary record added successfully."
      );
        ...
        <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-2 text-slate-400">
              Record an employee salary effective period in the
              confidential personnel system.
            </p>
          </div>
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-6">
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <div className="md:col-span-2">
                ...
                <p className="mt-2 text-xs text-slate-500">
                  Leave blank for a currently open-ended salary period.
                </p>
              </div>
            </div>

            <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-5 py-4 text-sm text-amber-200">
              Salary information is confidential personnel data and
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              >
                {loading ? "Saving Salary..." : "Save Salary Record"}
              </button>
            </div>
          </form>
        </section>
